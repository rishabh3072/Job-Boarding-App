...
const EmployerProfile = () => {
  ...
  const fetchJobs = async () => {
    try {
      const { data } = await axios.get("https://job-boarding-app-backend.onrender.com/api/v1/job/getmyjobs", { withCredentials: true });
      setJobs(data.myJobs);
    } catch (error) {
  ...
  const handleProfilePictureChange = (e) => {
    ...
    axios.post("https://job-boarding-app-backend.onrender.com/api/v1/user/uploadProfilePicture", formData, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await axios.put(
        "https://job-boarding-app-backend.onrender.com/api/v1/user/update",
        formData,
        { withCredentials: true }
      );
      setUser(data.user);
